...
The Android client: a landscape, voice-only terminal cockpit that attaches to sessions over `../specs/protocol.md` and renders rich TUIs with Termux's VT engine. Conventions from nothing-to-say (Kotlin 2.3, Compose + Material3, single-activity, singleton-model state, OkHttp).
...
1. Point the app at the backend in `local.properties` (gitignored) — the `remote_coding` service in `nothing-serious`, reached through the hosted relay:
   - `TERMINALS_BASE_URL=https://<host>/api/remote_coding`
   - `TERMINALS_DEVICE_TOKEN=<device token>`
2. Build and install: `./gradlew :app:installDebug`.
3. In the app: on the manager page, Add new terminal → pick a preset; it appears in the ring and renders the session. Swipe up to speak → Confirm injects the proposed action.
