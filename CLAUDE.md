...
1. This repo does **not** host its own `journal/`. Design and implementation notes live in the siblings: the backend brief is `../nothing-serious/journal/v20/2026-06-24_05_terminals.md` and the phone-side UI design is `../nothing-to-say/journal/002/2026-06-24_36_terminal_cockpit.md`. Read those for context; do not duplicate their content here.
2. `PROTOCOL.md` is the frozen wire contract shared with the `nothing-serious` backend. Treat it as the source of truth for frames and the back-channel; if a change is needed, change `PROTOCOL.md` first and flag that the backend must follow.
...
- `../nothing-serious` — the real HOME-only Python backend (Quart) that will implement the same `PROTOCOL.md` and serve real PTYs. It is the contract co-owner: protocol decisions here must stay compatible with its `terminals/` service. Models routed through preset tiers; Anthropic is integration-only.
...
- `PROTOCOL.md` fidelity is the point of this harness: if the server and the contract disagree, the client gets tested against a lie. Keep them in lockstep, and change `PROTOCOL.md` first when the contract itself must move.
